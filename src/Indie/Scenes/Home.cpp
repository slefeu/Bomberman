...
void Home::action() noexcept
{
    if (controller.isGamepadConnected(0)) {
        if (controller.isGamepadButtonPressed(0, G_Button::G_DPAD_UP) button_index > 0)
            button_index_ = button_index_ - 1;
        if (controller.isGamepadButtonPressed(0, G_Button::G_DPAD_DOWN) && button_index < buttons_.size() + settings_.size())
            button_index_ = button_index_ + 1;
        if (controller.isGamepadButtonPressed(0, G_Button::G_B)) buttons_[button_index_].action();
        for (auto& it : buttons_) it.setState(0);
        for (auto& it : settings_) it.setState(0);
        if (button_index_ > buttons_.size()) {
            settings_[button_index_ - buttons_.size()].setState(1);
        } else
